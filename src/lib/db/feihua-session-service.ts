 ...
 * 飞花令对局会话服务（服务端业务编排）
 *
 * - 开局：复用诗词语料调飞花令引擎生成轮次（含答案），入库，返回剥离答案的视图
 * - 判题：答案只从数据库读取，客户端提交的选择在服务端比对
 * - 计分：与诗词同款连击计分（纯函数），更新会话总分
 * 语料即诗词库（Poem 表 / poetry-seed），无需独立数据表。
 */

import { randomUUID } from "node:crypto";
import { ApiError } from "@/lib/crypto/with-crypto";
import { buildRounds } from "@/lib/games/feihua/engine";
...
export interface StartFeihuaSessionInput {
  difficulty?: string;
  count?: number;
...
export interface StartFeihuaSessionView {
...
export async function startFeihuaSession(
  input: StartFeihuaSessionInput,
): Promise<StartFeihuaSessionView> {
  ...
  if (input.playerId && useDb && STAGE_ORDER.FEIHUA.indexOf(difficulty) > 0) {
    await assertStageUnlocked(input.playerId, "FEIHUA", difficulty);
  }
  const corpus = await corpusPromise;
  const rounds = buildRounds(corpus, {
    difficulty,
    count,
    ...(input.playerId ? { excludeKeys: getSeenKeys(input.playerId, "FEIHUA") } : {}),
  });
  if (rounds.length === 0) {
...
export interface FeihuaAnswerInput {
  gameSessionId?: string;
  roundIndex?: number;
...
export interface FeihuaAnswerView {
  correct: boolean;
  /** 是否超时未答（按答错处理） */
  timeout?: boolean;
...
export async function judgeFeihuaAnswer(
  input: FeihuaAnswerInput,
): Promise<FeihuaAnswerView> {
  ...
  if (useDb) {
    ...
    if (!session || session.mode !== "FEIHUA") throw new ApiError(404, "对局不存在");
    rounds = session.rounds as unknown as FeihuaRound[];
    ...
    if (!mem || mem.mode !== "FEIHUA") throw new ApiError(404, "对局不存在");
    rounds = mem.rounds as FeihuaRound[];
    status = mem.status;
  ...
  const { lingChar, sentence, poemTitle, poet, dynasty } = round.meta;

  const explanation =
    round.type === FeihuaQuestionType.GUESS_CHAR
      ? `「${sentence}」含「${lingChar}」字，可飞「${lingChar}」令，出自${dynasty}·${poet}《${poemTitle}》`
      : round.type === FeihuaQuestionType.FIND_MISSING
        ? `「${sentence}」中没有「${lingChar}」字，出自${dynasty}·${poet}《${poemTitle}》`
        : `「${sentence}」含「${lingChar}」字，出自${dynasty}·${poet}《${poemTitle}》`;
    ...
    if (sessionPlayerId && useDb) {
      ...
      const [prev] = await Promise.all([
        prisma.playerProgress.findUnique({
          where: {
            playerId_mode_stage: { playerId: sessionPlayerId, mode: "FEIHUA", stage: sessionStage },
          },
          select: { stars: true },
        }),
        updateProgress({
...
function normalizeDifficulty(raw?: string): FeihuaDifficulty {
  const value = raw?.toUpperCase();
  if (value === FeihuaDifficulty.NORMAL || value === FeihuaDifficulty.HARD) {
    return value as FeihuaDifficulty;
  }
  return FeihuaDifficulty.EASY;
}
